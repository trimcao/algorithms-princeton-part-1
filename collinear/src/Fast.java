/* Way to pass timing test:
* Use an aux array to store the sorted points based on SLOPE_ORDER
* This way works because the slope to the same point will be -Infinity
* Whenever I see a segment, just check the start of the segment with the ref
* If the start of the segment < ref ==> duplicate segment
*/
  ...
  public static void main(String[] args) {
    ...
    */
    ...
    while (index < N) {
      ...
      /* // for debugging
      for (int i = 1; i < N; i++) {
        System.out.println(list[i].toString() + " and slope = " + ref.slopeTo(list[i]));
      }
        ...
        if (isPrint) {
          ...
          for (int j = 0; j < slopeList.size(); j++) {
            ...
            if (slopeList.get(j) == slope) {
              double checkSlope;
              for (int k = 0; k < preOut.length; k++) {
                if (pointList.get(j).compareTo(preOut[k]) != 0) {
                  temp = preOut[k];
                  break;
                }
              }
              if (temp != null) {
                ...
                if (pointList.get(j).compareTo(temp) > 0) {
                  checkSlope = temp.slopeTo(pointList.get(j));
                }
                else {
                  checkSlope = pointList.get(j).slopeTo(temp);
                }
                /*
                if (pointList.get(j).compareTo(temp) == 0) // may actually need to compare every point in pre OUt
